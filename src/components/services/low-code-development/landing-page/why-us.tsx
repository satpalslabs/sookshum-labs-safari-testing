  ...
  return (
    <div>
      ...
      >
        ...
        >
          <div className="w-full text-wrap xs:w-full text-center mx-auto xs:text-wrap">
            Why Sookshum Labs
          </div>
        </SectionHeader>
...
const ChooseUsCards: React.FC<{ isInView: boolean }> = ({ isInView }) => (
  <div className="w-full flex gap-5 h-full max-h-full xs:flex-col-reverse">
    <Card
...
const ChooseUsCard: React.FC<{ item: ChooseUs }> = ({ item }) => (
  <div className="flex gap-5 lg:gap-3 sm:gap-2 w-full items-start">
    ...
    <div className="flex flex-col gap-3 sm:gap-1  pt-1">
      ...
      <Description text={item.description} classes={"grow !font-normal text-lg"} children={null} />
    </div>
  </div>
);
